// ViajeAqui - Frontend sem <div>, com PWA, Leaflet e recursos locais
...
// Conversor de moedas (exchangerate.host)
let taxas = JSON.parse(localStorage.getItem('viajeaqui_taxas')||'{}');
async function atualizarTaxas(base='USD'){
  try{
    const r = await fetch(`https://api.exchangerate.host/latest?base=${base}`);
    const j = await r.json();
    taxas[base] = j.rates;
    localStorage.setItem('viajeaqui_taxas', JSON.stringify(taxas));
    alert('Taxas atualizadas!');
  }catch(e){ alert('Falha ao atualizar taxas.'); }
}
...
document.getElementById('form-cambio')?.addEventListener('submit', (e)=>{
  ...
  if (!baseRates) { alert('Atualize as taxas primeiro.'); return; }
